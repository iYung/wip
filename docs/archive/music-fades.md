...
- [x] Task A — `lua/game/sound.lua` — Replace the single `_music` variable with `_music_tracks = {}`. Each entry keyed by track name (`"menu"`, `"bg"`) holds `{ src, fade_vol, fade_target, fade_rate, stop_on_done }`. In `Sound.load()`, load `"assets/music/menu.wav"` as a looping `"stream"` source into `_music_tracks["menu"]` (set `fade_vol=1`, `fade_target=1`, `fade_rate=0`, `stop_on_done=false`) and play it immediately at `_music_volume`. Load `"assets/music/background.wav"` the same way into `_music_tracks["bg"]` but do NOT play it (leave it stopped and silent). Remove the old `_music` variable and its `love.audio.play` call.
