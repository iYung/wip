...
- [x] Update `start_scene.lua` to load each PNG via `try_img`, fall back to current code-drawn style if missing
- [x] Create `start_bg.png` — wire up in scene
- [x] Create `start_logo.png` — wire up in scene
- [x] Create `start_btn.png` and `start_btn_selected.png` — wire up in scene
- [x] Clean up fallback code once all PNGs are in place
