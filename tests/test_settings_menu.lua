...
love.keyboard.isDown = function(k) return k == "i" end
...
-- Test 28: Sub-screen down wraps from row 8 (Return) to row 1
open_clean(m)
m._subscreen = "keybinds"
m._subscreen_selected = 8
m._prev_sub_down = false
sim_key(m, "s")
assert(m._subscreen_selected == 1, "down from row 8 should wrap to row 1, got " .. m._subscreen_selected)
print("PASS: sub-screen down wrap")

-- Test 29: Sub-screen up from row 1 wraps to row 8 (Return button)
open_clean(m)
m._subscreen = "keybinds"
m._subscreen_selected = 1
m._prev_sub_up = false
sim_key(m, "w")
assert(m._subscreen_selected == 8, "up from row 1 should wrap to row 8, got " .. m._subscreen_selected)
print("PASS: sub-screen up wrap to Return")

-- Restore move_left cleared by the collision test (Test 26) so _all_bound passes
state.keybinds.move_left = "a"

-- Test 30: Confirming Return button (row 8) exits sub-screen, menu stays open
open_clean(m)
m._subscreen = "keybinds"
m._subscreen_selected = 8
...
open_clean(m)
clear_sounds()
sim_key(m, "i")   -- confirm on Fullscreen/Window (index 1)
...
do
    ...
    m48._subscreen_selected = 8   -- Return row = #_ACTION_LIST + 1
