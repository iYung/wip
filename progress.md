...
- **Inter font** — replaced Love2D's default Vera font with `assets/fonts/font.ttf` (Inter Variable) across all text-rendering sites; `lua/game/fonts.lua` centralises the path and hinting mode (`"light"`); `main.lua` sets it as the default font so `StoreScene` HUD and customer speech bubbles inherit it automatically; `buy_scene`, `settings_menu`, and `start_scene` use `Fonts.new(size)` for their explicit fonts
